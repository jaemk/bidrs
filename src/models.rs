...
trait FromRow<T> {
    fn from_row(row: postgres::rows::Row) -> T;
}
...
impl FromRow<User> for User {
    fn from_row(row: postgres::rows::Row) -> User {
        User {
            id: row.get(0),
            auth_id: row.get(1),
            email: row.get(2),
...
impl FromRow<Organization> for Organization {
    fn from_row(row: postgres::rows::Row) -> Organization {
        Organization {
            id: row.get(0),
            name: row.get(1),
            extra: row.get(2),
...
impl FromRow<Bidder> for Bidder {
    fn from_row(row: postgres::rows::Row) -> Bidder {
        Bidder {
            id: row.get(0),
...
impl FromRow<PaymentInfo> for PaymentInfo {
    fn from_row(row: postgres::rows::Row) -> PaymentInfo {
        PaymentInfo {
            id: row.get(0),
            cc_number: row.get(1),
            cc_pin: row.get(2),
...
impl FromRow<Profile> for Profile {
    fn from_row(row: postgres::rows::Row) -> Profile {
        Profile {
            id: row.get(0),
            user_id: row.get(1),
            bidder_id: row.get(2),
...
impl NewProfile {
    pub fn new(user_id: i32, bidder_id: Option<i32>, payment_info_id: Option<i32>,
               level: i32, is_primary: bool, name: &str, phone: Option<&str>,
               extra: Option<Json>) -> NewProfile {
        NewProfile {
            user_id: user_id, bidder_id: bidder_id, payment_info_id: payment_info_id,
            level: level, is_primary: is_primary, name: name.into(), phone: phone.map(|p| p.into()),
            extra: extra,
        }
    ...
    pub fn create(self, conn: &Connection) -> Result<Profile> {
        ...
        try_insert_to_model!(conn.query(qs, &[&self.user_id, &self.bidder_id, &self.payment_info_id,
                                              &self.level, &self.is_primary, &self.name,
                                              &self.phone, &self.extra]) ;
                             Profile ;
                             id: 0, date_created: 1, date_modified: 2 ;
                             user_id: self.user_id, bidder_id: self.bidder_id,
                             payment_info_id: self.payment_info_id, level: self.level,
...
impl FromRow<Item> for Item {
    fn from_row(row: postgres::rows::Row) -> Item {
        Item {
            id: row.get(0),
            organization_id: row.get(1),
...
impl FromRow<Bid> for Bid {
    fn from_row(row: postgres::rows::Row) -> Bid {
        Bid {
            id: row.get(0),
            bidder_id: row.get(1),
            item_id: row.get(2),
